...
// The congratulations note, on its own key so the admin gets a dedicated entry
// in the site editor for it.
//
// Anything wrapped in class="copyable" becomes click-to-copy on the dashboard:
// components/EditableCopy.js injects a "copy" chip after it, and clicking
// either the sentence or the chip copies it and shows a green tick. The wording
// is fully editable — whatever the admin writes is what players copy.
//
// This lives on its own key rather than inside dashboard_html on purpose:
// getContent() prefers a saved database row over the default, so folding it
// into an already-saved block would have left it invisible with nothing to
// edit. A fresh key has no saved row, so its default always shows through.
export const DEFAULT_DASHBOARD_NOTE_HTML = `<p><span class="copyable">Congratulations for your earnings.</span></p>`;
...
export const DEFAULTS = {
  ...
  dashboard_note_html: DEFAULT_DASHBOARD_NOTE_HTML,
...
export async function getDashboardNoteHtml() {
  return getContent("dashboard_note_html");
}
